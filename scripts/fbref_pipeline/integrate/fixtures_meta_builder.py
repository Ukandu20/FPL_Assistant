...
plus hex IDs (`home_id`, `away_id`, *and* now a single‐row **team_id**).
...
def build_fixture_calendar(season: str, fpl_csv: Path, fb_csv: Path,
    ...
    # ---------- look-ups
    name2hex,name2code,code2hex = build_maps(load_json(team_map_fp),
                                             load_json(short_map_fp))
    ...
    for side in ("home","away"):
        ...
        fpl[f"{side}"]      = fpl[f"{side}_long"].map(name2code)   # short code
        fpl[f"{side}_hex"]  = fpl[f"{side}"].map(code2hex)

    # ---------- FBref (match schedule) --------------------------------------------
    fb = pd.read_csv(fb_csv,  parse_dates=["game_date"])
    #fb  = fb[fb.get("is_home", 1) == 1].copy()      # one row per game
    fb["date_played"] = normalise_date(fb["game_date"])
    fb_match = fb[["game_id","team","team_id",
                   "home","away","date_played",
                   "venue","ga","gf","xga","xg","result",
                   "is_promoted","is_relegated"]]
    ...
    # derive team_id (hex) from short code when the merge didn’t get it
    if "team_id" not in cal.columns:
        cal["team_id"] = cal["team"].map(code2hex)
    else:
        cal["team_id"].fillna(cal["team"].map(code2hex), inplace=True)
...
def main():
    ...
    seasons = [args.season] if args.season else \
              sorted(d.name for d in args.fpl_root.iterdir() if d.is_dir())
    if not seasons:
