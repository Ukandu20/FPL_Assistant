...
def fpl_selling_price(_purchase_price: float, current_price_val: float) -> float:
    ...
    Market-to-market rule:
      sell at the current snapshot market price (rounded down to 0.1).
    """
...
def swap_transfer_from_master(
    ...
    *,
    ...
    """
    Atomic sell+buy with pre-checks:
      - bank must remain >= 0 (precomputed from proceeds and cost)
      - composition must remain exactly 2/5/5/3 after swap
      - one combined log entry
    """
    if out_id == in_id:
    ...
    # Preconditions: current composition valid (so swap keeps it valid if positions unchanged)
    if len(state.squad) == 15:
        # Only enforce when full squad present
        validate_exact_composition(state)

    # Compute proceeds (out) and cost (in) WITHOUT mutating state
    ...
    # Locate out player and its locked buy_price (for completeness, but sell is mark-to-market)
    ...
    # Pre-check composition after swap (only enforce if squad will remain 15)
    new_players = state.squad.copy()
    # replace out with in (same index)
    new_players[i_out] = SquadEntry(
        player_id=in_id,
        name=None,  # not needed for composition check
        pos=pos_in,
        team_id="",
        buy_price=0.0,
        sell_price=0.0,
...
def fh_begin(state: TeamState, *, gw: Optional[int] = None, chips_log: bool = True) -> None:
    ...
    if chips_log:
        # If FH available, consume it and log usage on the specified GW or upcoming
...
def _build_parser():
    ap = argparse.ArgumentParser(
        description="Manage team state (master integration, snapshots, transactions, composition checks, Free Hit backup)"
    )
    ...
    # swap-from-master
    p = sub.add_parser("swap-from-master", help="swap out->in atomically at snapshot gw; enforces bank>=0 & composition")
    p.add_argument("path", help="team_state.json path")
    p.add_argument("--master", required=True, help="path to master_fpl.json")
