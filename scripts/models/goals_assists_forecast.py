...
• Trained artifacts from goals_assists_model_builder.py:
    - artifacts/features.json (authoritative feature order)
    - goals_global_lgbm.txt (+ optional per-pos: goals_GK/DEF/MID/FWD_lgbm.txt)
    - assists_global_lgbm.txt (+ optional per-pos variants)
    - Optional Poisson heads: goals_global_poisson.joblib, assists_global_poisson.joblib (+ per-pos)
    - Optional calibration: artifacts/p_goal_isotonic_per_pos.joblib, artifacts/p_assist_isotonic_per_pos.joblib
    - meta.json (captures args used when training — halflives, etc.)

• Minutes forecast CSV from minutes_forecast.py:
    - Must include: season, player_id, team_id, pos, date_sched, pred_minutes,
                    p_start, p_cameo, pred_start_head, pred_bench_cameo_head,
                    gw_orig or gw_played (prefer gw_played>0 else gw_orig>0 else gw)
    - Also includes fdr (already merged by minutes_forecast if --use-fdr)

• Registry features:
    - players_form.csv for history seasons + current season up to --as-of
    - team_form.csv (for team_att_z_venue / opp_def_z_venue by GW)

Key guarantees
--------------
• No retraining.
• No future rows are used to compute EWM/recency (strict < --as-of).
• Feature shape and order are pinned to artifacts/features.json → no LGBM shape errors.
...
def _team_z_venue(team_form: Optional[pd.DataFrame]) -> Optional[pd.DataFrame]:
    ...
    if {"team_att_z_venue","opp_def_z_venue"}.issubset(team_form.columns):
        ...
        return t
    # fallback from split z-scores if available
...
def _last_snapshot_per_player(df: pd.DataFrame, feature_cols: List[str], as_of_ts: pd.Timestamp, tz: Optional[str]) -> pd.DataFrame:
    ...
    Robust to absence of any GW column (gw_played/gw_orig/gw)."""
    ...
    # add placeholders for any missing feature to be filled later
...
def _predict_reg(booster: Optional[lgb.Booster], X: pd.DataFrame) -> np.ndarray:
    ...
    # LightGBM Booster expects numeric matrix in the trained order; we'll pass values in feature order.
...
def _predict_poisson_per_pos(name: str,
    ...
    used = np.zeros(len(X), dtype=bool)
    # Ensure strictly numeric matrix with no NaNs (Poisson heads require this)
...
def main():
    ...
    # EWMA halflife per pos (fallbacks mirror trainer defaults)
    ...
    # shots/SOT EWMs computed leak-free on history+played rows only
    ...
    # Cast NumPy ints → built-in ints now to avoid JSON issues later
    ...
    # restrict minutes rows to target_gws
    ...
    minutes = minutes.merge(vmap, how="left", on=["season","team_id",gw_key_m])

    # Build future frame (one row per player fixture)
    fut = minutes.copy()
    fut["venue_bin"] = fut["is_home"].fillna(0).astype(int)
    # we'll trust minutes fdr; else fill 0
    ...
    if tz_map is not None and (gw_key_m in fut.columns):
        fut = fut.merge(tz_map.rename(columns={"gw_orig": gw_key_m}),
                        how="left", on=["season", gw_key_m, "team_id"])
    else:
        fut["team_att_z_venue"] = np.nan
        fut["opp_def_z_venue"] = np.nan

    # Last-known player snapshot for EWM/ROLL & recency
    # Take only columns that might appear in features.json (e.g., *_ewm, *_roll, is_active, minutes→prev_minutes),
    # AND preserve any GW columns for robust sorting.
    pull_cols = set([c for c in pf_hist.columns if ("_ewm" in c or "_roll" in c)])
    pull_cols |= {"is_active","minutes"}  # minutes used to derive prev_minutes
    # we will also compute prev_minutes & days_since_last below
    # we will also compute prev_minutes & days_since_last below
    ...
    # days_since_last: difference between fixture date and last played date per player
    # compute last_play_date per (season, player_id)
    ...
    # prev_minutes / is_active from last snapshot
    # (last contains minutes→we map to prev_minutes; is_active last known)
    ...
    if fut.columns.duplicated().any():
        ...
        logging.warning(f"Duplicate columns detected; keeping last occurrence: {set(dups)}")
    ...
    def _get_unique_col(df: pd.DataFrame, name: str) -> pd.Series:
        """Return a single Series for 'name', even if duplicates exist."""
        obj = df[name]
        # If duplicates slipped through somehow, reduce to the rightmost
        if isinstance(obj, pd.DataFrame):
            # select by column label (rightmost) to avoid tuple-based indexing that some type-checkers flag
    ...
    # median vector for numeric cols; fill med NaNs with neutral 0
    ...
    scale = fut["pred_minutes"].to_numpy() / 90.0
    ...
    # Choose rates for probability policy (prefer Poisson if available)
    ...
    # Optional isotonic calibration (use saved per-pos calibrators if available)
    ...
    # Zero GK if requested for FPL decisions
