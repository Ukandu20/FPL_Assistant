...
# -*- coding: utf-8 -*-
r"""
points_forecast.py — upcoming fixtures — v1.2
...
def _norm_cols(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Parse any commonly used date columns; do NOT normalize to day (we want real times for tie-break)
...
def _expected_floor_div3_poisson(lam: np.ndarray, kmax: int | None = None) -> np.ndarray:
    # sanitize input: replace NaN/Inf/negatives with 0
...
# Force 4-col key (DGW-ambiguous by itself; we mitigate with nearest-date tie-break)
FORCED_KEY = ["season", "gw_orig", "player_id", "team_id"]

# Candidate date columns used ONLY for tie-breaking / sorting (prefer scheduled, then played, then kickoff)
...
def _merge_with_nearest_date(
    ...
    """Left-merge on 4-col key; if duplicates from `right`, keep the row whose right-date is
    nearest to the base-date (if both sides have a datetime column). Falls back to 'last'."""
    ...
    # Build right subset with optional right-date column
    ...
    # If no duplicates for any base row, or no date columns to resolve, collapse by 'last'
    ...
    # Resolve duplicates by nearest absolute delta
    dup_counts = m["__rowid__"].value_counts()
    multi = int((dup_counts > 1).sum())
    if multi:
        logging.info("[%s] duplicate 4-col matches for %d base rows; resolving via nearest-date", tag, multi)
...
def main():
    ...
    #   • Minutes: keep as-is (one row per player–fixture expected; do not squash per 4-col key)
    #   • Aux tables: drop exact dup rows; allow multi-matches per key (DGW) and resolve on merge
    ...
    # Base = MINUTES (guard for key presence)
    ...
    # GA expectations (pred_goals_mean, pred_assists_mean)
    ...
    lam_team = lam90.copy()
    lam_team = lam_team.where(lam_team.notna(), exp_gc)
    ...
    # Saves EP (GK only) — exact E[floor(S/3)]
    ...
    if had_pm:
        lam_saves = raw_lambda
    else:
        lam_saves = raw_lambda * np.clip(m/90.0, 0.0, None)
    ...
    out_cols = list(dict.fromkeys([
        ...
        *comp_cols, "xPts",
        # include dates if present (useful for inspection/sorting)
        *[c for c in LEFT_DATE_PREFS if c in base.columns]
    ]))
    out_cols = [c for c in out_cols if c in base.columns]

    # Sort: 4-col key + dates for stable per-fixture ordering
    sort_cols = [c for c in (KEY + ["team_id","player_id"]) if c in base.columns]
