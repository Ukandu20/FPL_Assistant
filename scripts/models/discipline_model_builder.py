#!/usr/bin/env python3
r"""
discipline_model_builder.py – v1 (corrected)

Predict negative FPL point components:
  • Yellow card (YC)            : –1 point
  • Red card (RC)               : –3 points
  • Own goal (OG)               : –2 points
  • Missed penalty (MPEN)       : –2 points
  • Goals conceded penalty (GC) : –1 per 2 conceded (GK/DEF only)

Key improvements:
  • Missing minutes on TEST are IMPUTED (train-only medians × play-rate priors). No dropping.
  • Robust alignment: keep TEST as a single slice; no mask/index mismatches.
  • LightGBM trained & predicted with DataFrames → no feature-name warnings.
  • GC penalty uses minutes-aware Poisson expectation:
        If λ = team GA rate and m = expected minutes,
        λ_on = λ * (m/90),  E[floor(N/2)] = λ_on/2 − (1 − e^(−2λ_on))/4,
        points = −E[floor(N/2)] for GK/DEF only.
  • Defense preds accepted as:
        pred_ga_lambda  (preferred), or
        pred_ga_mean     (treated as λ), or
        cs_prob_cal/raw  (λ = −log P(CS)).

CLI (example)
  python -m scripts.models.discipline_model_builder \
    --features-dir data/processed/features \
    --version v9 \
    --test-season 2024-2025 \
    --test-last-n 10 \
    --minutes-preds data/models/expected_minutes/v9/minutes_predictions.csv \
    --defense-preds data/models/defense/v6/predictions/defence_predictions.csv \
    --poisson-heads \
    --models-out data/models/discipline --model-version v1
"""
...
def _load_minutes_predictions(path: Optional[Path]) -> Optional[pd.DataFrame]:
    ...
    if "date_played" in mp.columns:
        ...
        logging.warning("minutes_preds missing date_played; merge may fail.")
...
def _load_defense_predictions(path: Optional[Path]) -> Optional[pd.DataFrame]:
    ...
    if "pred_ga_lambda" in have:
        ...
        # Derive λ from clean-sheet probability if available: λ = −log P(CS)
        p_cs = None
...
def _merge_team_z(df_players: pd.DataFrame, team_form_dir: Path, version: str) -> pd.DataFrame:
    ...
    # Prefer prebuilt columns; else build from *_home/away*_roll_z
    has_pre = {"team_def_z_venue","opp_att_z_venue"}.issubset(tf.columns)
    if has_pre:
        ...
        t["team_def_z_venue"] = np.where(
            t["venue"] == "Home", t["def_xga_home_roll_z"], t["def_xga_away_roll_z"]
        )
        t["opp_att_z_venue"] = np.where(
            t["venue"] == "Home", t["att_xg_away_roll_z"], t["att_xg_home_roll_z"]
        )
...
def _build_features(df: pd.DataFrame, use_z: bool, na_thresh: float) -> Tuple[pd.DataFrame, List[str]]:
    ...
    # optional rolling features related to discipline if available
...
def main():
    ...
    # Default False: we impute
    ap.add_argument("--drop-missing-minutes", action="store_true",
                    help="Drop test rows with missing pred_exp_minutes (default False; we impute instead)",
                    default=False)
    ...
    df["date_played"] = df["date_played"].dt.date

    # Attach team z
    df = _merge_team_z(df, args.team_form_dir, args.version)

    # Targets: pick columns (create zeros if absent)
    ...
    # Build features (mutates df to include prev_minutes)
    ...
    # Split (keep DataFrames to preserve feature names)
    train_idx, test_idx = _time_split_last_n(df, args.test_season, args.test_last_n)
    Xtr, Xte = X.loc[train_idx], X.loc[test_idx]

    y_yc_tr = df.loc[train_idx,"y_yc_p90"].to_numpy()
    y_rc_tr = df.loc[train_idx,"y_rc_p90"].to_numpy()
    y_og_tr = df.loc[train_idx,"y_og_p90"].to_numpy()
    y_mp_tr = df.loc[train_idx,"y_mp_p90"].to_numpy()
    ...
    # Base test slice
    base = df.loc[test_idx, KEY + ["player","pos","venue"]].reset_index(drop=True).copy()
    for c in ("season","player_id","team_id"):
        base[c] = base[c].astype(str).str.lower().str.strip()
    base["date_played"] = pd.to_datetime(base["date_played"]).dt.date
    ...
    # ------------------ Train-only baselines for imputation (CAUSAL) ------------------
    ...
    med_team_pos = (train_df.groupby(["season","team_id","pos"])["minutes"]
                    .median().rename("med_min_team_pos"))
    med_pos_season = (train_df.groupby(["season","pos"])["minutes"]
                      .median().rename("med_min_pos_season"))
    med_pos_global = (train_df.groupby("pos")["minutes"]
                      .median().rename("med_min_pos_global"))
    med_global = float(train_df["minutes"].median())

    rate_team_pos = (train_df.groupby(["season","team_id","pos"])["played1"]
                     .mean().rename("p1_rate_team_pos"))
    rate_pos_season = (train_df.groupby(["season","pos"])["played1"]
                       .mean().rename("p1_rate_pos_season"))
    rate_pos_global = (train_df.groupby("pos")["played1"]
                       .mean().rename("p1_rate_pos_global"))
    ...
    # p1 estimate (hierarchical fallback)
    ...
    prev_minutes_test = df.loc[test_idx, "prev_minutes"].reset_index(drop=True)
    p1_est = p1_est.fillna((prev_minutes_test.fillna(0) > 0).astype(float))
    p1_est = p1_est.fillna(rate_global)

    # median minutes baseline (hierarchical fallback)
    ...
    # Impute missing pred_exp_minutes
    missing_mask = merged["pred_exp_minutes"].isna()
    imputed_exp_minutes = (p1_est * med_base).astype(float)
    merged.loc[missing_mask, "pred_exp_minutes"] = imputed_exp_minutes[missing_mask]
    num_imputed = int(missing_mask.sum())
    if num_imputed:
        logging.info("Imputed exp_minutes for %d/%d test rows (train-only baselines).",
                     num_imputed, len(merged))

    # Build final p1 to use downstream
    ...
    p1 = p1.fillna((prev_minutes_test.fillna(0) > 0).astype(float))
    ...
    # --- GC penalty (exact Poisson, minutes-aware; no extra p1 factor) ---
    ...
    # E[floor(N/2)] for Poisson N~Pois(lam_on): λ/2 − (1 − e^(−2λ))/4
    ...
    # Diagnostics
    y_true_yc = pd.to_numeric(df.loc[test_idx, col_yc], errors="coerce").fillna(0).to_numpy()
    y_true_rc = pd.to_numeric(df.loc[test_idx, col_rc], errors="coerce").fillna(0).to_numpy()
    y_true_og = pd.to_numeric(df.loc[test_idx, col_og], errors="coerce").fillna(0).to_numpy()
    y_true_mp = pd.to_numeric(df.loc[test_idx, col_mp], errors="coerce").fillna(0).to_numpy()
